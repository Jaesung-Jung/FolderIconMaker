...
struct TahoeEmbossRenderer {
  func render(base: CGImage, symbol: CGImage, settings: RenderSettings) -> CGImage {
    ...
    let mask = SymbolImageLoader.alphaMask(
      from: symbol, canvasSize: size, targetRect: settings.symbolRect)
    ...
    for y in bounds.minY..<bounds.maxY {
      for x in bounds.minX..<bounds.maxX {
        ...
        let shadow =
          CGFloat(
            mask.value(
              x: x - Int(settings.shadowOffset.width), y: y - Int(settings.shadowOffset.height)))
          / 255.0
        ...
        if highlight > 0 {
          blendScreen(
            &pixels, index: index, color: (189, 238, 255),
            amount: highlight * settings.highlightOpacity)
        }

        if glow > 0 {
          blendScreen(
            &pixels, index: index, color: (169, 232, 255), amount: glow * settings.glowOpacity)
        }
      }
    }
  ...
  private func expandedEffectBounds(settings: RenderSettings, canvasSize: Int, glowRadius: Int)
    -> EffectBounds
  {
  ...
  private func blendScreen(
    _ pixels: inout [UInt8], index: Int, color: (CGFloat, CGFloat, CGFloat), amount: CGFloat
  ) {
